...
contract ChessBetting {
    ...
    // Get dispute status
    function getDisputeStatus(uint256 _betId)
        external
        view
        returns (bool player1Disputed, bool player2Disputed, bool canDispute)
    {
        Bet memory bet = bets[_betId];
    ...
    function isBetExpired(uint256 _betId) external view returns (bool) {
        Bet memory bet = bets[_betId];
        return bet.status == BetStatus.Created && block.timestamp > bet.createdAt + betExpiryTime;
    }
